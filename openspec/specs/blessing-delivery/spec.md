# Blessing Delivery Specification

## Purpose

管理一条祝福从提交到被看见的全过程：作者提交即"已发送"并拿到可分享链接，平台先校验再让接收方可见；访客免注册按祝福状态查看正文或中性占位；作者可撤回（终态，只能删除或复制另发新的）、删除、续期。
...
作者 SHALL 可查看自己发布的祝福列表（时间、场景、给谁、状态）。撤回 MUST 立即停止公开访问，且 MUST 是终态——撤回后的祝福 MUST NOT 再被重新发布或重新投递；作者若想再送一次相近的内容，只能复制正文到一份全新的草稿另行提交（走完整的提交 / 校验 / 投递流程，产生新的祝福记录）。删除 MUST 二次确认、按不可逆处理，删除后按数据保护要求冻结数据（停止除存储与安全外的处理），且 MUST NOT 仍出现在作者列表。撤回 / 删除 / 过期 / 下架对公开落地页的生效 MUST 在 60 秒内完成（含缓存失效）。

#### Scenario: 撤回后不能重新发布

- **WHEN** 作者撤回一条祝福
- **THEN** 系统不提供任何"重新发布"这条已撤回记录的操作，该记录保持 `withdrawn`，唯一可执行的操作是删除

#### Scenario: 复制正文另行提交

- **WHEN** 作者对一条已撤回的祝福选择"复制以供编辑"
- **THEN** 系统把正文预填进一份新的撰写草稿，作者可修改后作为一条全新的祝福提交；原撤回记录不受影响、不产生新的投递
...
一条回复祝福 SHALL 记录它是在回哪一条原始祝福。原发送者的收件箱在展示这条回复时 SHALL 附带原信的关联信息（至少含原信正文的预览），让发送者在群发多条的情况下也能分清一条回信对应哪条。系统 MUST 校验该关联指向的原信确实存在、且当前回复者确实是其收件人之一；不满足时 MUST 忽略这个关联（回复本身仍照常投递），不得凭调用方自报的关联直接采信。

#### Scenario: 收件箱展示回信关联的原文

- **WHEN** 原发送者收到一条回信，这条回信在提交时关联了自己此前群发的某条祝福
- **THEN** 收件箱在这条回信旁展示所关联原信的正文预览

#### Scenario: 伪造的关联被忽略

- **WHEN** 回复者提交时声称自己在回一条其实从未送达给自己的祝福
- **THEN** 系统不建立这个关联，回信照常投递但不附带原信预览
