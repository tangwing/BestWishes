...
#### Scenario: 音频回应在福袋里可播放，转写文字不脱离录音单独出现（B-93）

- **WHEN** 祈福作者在福袋里点开一条音频回应
- **THEN** 系统 SHALL 同时给出可播放的音频控件和转写文字，并明确标注这段文字是"这段录音的文字记录"——不能只展示转写文字、让它看起来像一条与录音无关的独立文字祝福
