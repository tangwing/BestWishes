...
### Requirement: 回应下的往返回复

一条祈福不是一次性的"求-应"，而应像社区 Topic 一样支持往返对话（B-89）。祈福作者与该回应的作者双方 SHALL 能就一条回应继续互相回复（文字，复用 `blessing-delivery` 的 `scope='reply'` 回信机制，不新建一套独立的评论系统）；祈福详情视图 SHALL 在每条回应下按时间正序展示这条回应引出的全部 `published` 回复，形成一条连续的对话记录。未过审 / 已撤回的回复 MUST NOT 出现在这条记录里。回复本身的安全审核、hold、投递均复用 `blessing-delivery` 现有规则，不额外定义。

#### Scenario: 请求人回复一条回应

- **WHEN** 祈福作者对一条已收到的音频回应发一段文字回复
- **THEN** 通过审核与 hold 后，这条回复出现在该回应下方，回应者能在自己的福袋和这个详情页两处看到

#### Scenario: 回应者接着回过去，形成往返

- **WHEN** 回应者对请求人刚发的回复再回一段
- **THEN** 详情页里这条回应下方按时间正序显示两条回复，读起来是连续的对话

#### Scenario: 非当事人不能在此发起回复

- **WHEN** 一个既不是祈福作者、也不是该回应作者的第三方查看详情
- **THEN** 页面不提供在这条回应下发起回复的入口
