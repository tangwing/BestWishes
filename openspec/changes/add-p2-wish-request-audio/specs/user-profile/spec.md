...
个人空间 SHALL 向用户本人展示一个"你已传递 N 份善意"的累计数字，N 为该用户当前处于 `published` 状态的祝福总数（不分自然日、不计连续天数）。这个数字 MUST 只对用户本人可见，MUST NOT 提供给其他用户或访客，MUST NOT 被转化为积分、等级、排行或任何可变现物。
...
- **WHEN** 用户打开个人空间
- **THEN** 看到"你已传递 N 份善意"，N 等于自己当前有效（`published` 且未因撤回/删除/下架失效）的祝福数
