...
- [ ] 9.1 领域层：`WishRequest` 加 `responseCount: number` / `lastResponseAt: string | null`；删除 `packages/domain/src/streak.ts` + 其测试；`pnpm test:unit` 绿
- [ ] 9.2 数据层：`wish_requests` 加 `response_count`（int not null default 0）/ `last_response_at`（timestamptz null）；drizzle 迁移；内存仓储同步；对账测试（跑完一段回应生命周期后 `response_count` == `COUNT(published wish_response)`）
- [ ] 9.3 服务端：回应进/出 `published` 时维护聚合列，维护点收敛到 `audio-scoring-service` / 状态落地处；`GET /api/plaza`（列表：摘要 + 统计，支持 `?filter=mine`）、`GET /api/plaza/:id`（详情：处境全文 + 稿子 + 全部 published 回应，无评分细节）取代 `GET /api/wish-requests` + `/wish-requests/:id/responses`；删 `wish-request-service` 里"我的请求"独立方法（并入 plaza 查询）；累计善意数（count published where author）进 `GET /api/me` 或 profile 接口；删 streak 路由 / 服务
- [ ] 9.4 前端：`WishRequests.tsx` → 祈福广场（列表只渲染摘要 + "已收到 N 条回应"，`?filter=mine` 筛选）；新增祈福详情页（`/plaza/:id`，处境 + 稿子 + 回应列表 + 逐条播放）；`RespondToWishRequest` 路由改 `/plaza/:id/respond`；`Compose.tsx` 重做为"传递善意"页并入 `Records.tsx` 的发件箱内容（路由 `/give`，发送确认页 `/give/sent/:id`）；`Inbox.tsx` 导航名改"我的福袋"（路由 `/pouch`）；删 `Streak.tsx` + `/streak` 路由；`Profile.tsx` 展示"你已传递 N 份善意"；`App.tsx` 导航 8 → 6 项（首页 · 祈福广场 · 传递善意 · 我的福袋 · 个人空间 · 审核台）；`main.tsx` 路由表按新命名重写，旧路径不做重定向（内测期，直接换）
- [ ] 9.5 测试：广场列表断言"不含回应正文 / 音频 URL"、详情断言"点进去才有回应"、`responseCount` 对账断言；e2e 全量改新路由 + 新导航文案（`wish-request.spec.ts` / `author-flow.spec.ts` 等）；删 streak 测试
- [ ] 9.6 文档：`docs/DEMO.md` 走查表按新导航 / 新路由重写；`docs/product/use-cases.md` 若提到"回响"一并清理
- [ ] 9.7 spec 已在本轮 `/opsx:update` 同步：`wish-request` delta（广场列表 / 我的祈福筛选 / 回应数聚合 / 查看详情 / 回应投递到福袋）、新增 `blessing-records` MODIFIED delta（发件箱并入传递善意、收件箱入口改名福袋）、新增 `user-profile` MODIFIED delta（去坚持记录入口、加累计善意数）、`blessing-streak` 整体 REMOVED delta。`openspec validate --strict` 通过
- [ ] 9.8 `pnpm verify` / `pnpm test:e2e` 全绿；BACKLOG（B-71 挪 CHANGELOG）/ CHANGELOG / PROMPT_LOG 更新
