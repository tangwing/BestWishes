...
- [x] 6.1 应用骨架、`createBrowserRouter` 路由、`SessionProvider`（/api/me）、`api/client.ts` 唯一 fetch 封装、`theme.css` token
- [x] 6.2 登录页（昵称即可）；未登录访客看 `/p/:slug` 无门槛，进 `/compose` 等自动跳登录
- [x] 6.3 个人空间：落款 / 城市预设、定位开关（占位）、精选展示个人偏好、坚持入口、退出
- [x] 6.4 授权协议页：分层勾选、精选默认态 = 个人偏好 > 系统默认
- [x] 6.5 范本参考（无一键填入、`user-select:none`）+ 自由创作；加载失败回退
- [x] 6.6 撰写页：`onPaste` 拦截 + 温和提示、三项发送者信息（默认取个人空间）、发心提示、呼吸圆环（`prefers-reduced-motion` 关）、字数提示
