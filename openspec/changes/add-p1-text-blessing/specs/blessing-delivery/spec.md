...
当一条祝福**首次**进入 `published` 时，系统 MUST 为它定格的每个收件人（见 `blessing-audience`）建一条收件箱条目，并触发一条通知（见 `notification`），然后标记该祝福已投递。已投递的祝福再次进入 `published`（如 `taken_down → published` 申诉恢复）MUST NOT 重复投递。校验期（`verifying`）MUST NOT 投递。

#### Scenario: 通过校验后投递

- **WHEN** 一条群发祝福 hold 结束、自动通过校验
- **THEN** 每个收件人的收件箱出现这条祝福，且各收到一条通知

#### Scenario: 申诉恢复不重复投递

- **WHEN** 一条已投递的祝福被下架后申诉成功、恢复 `published`
- **THEN** 收件箱里那条恢复为可见，但不新增收件箱条目或通知

### Requirement: 收件箱按状态渲染

收件箱条目 MUST 根据关联祝福的**当前**状态渲染：仅 `published` 且未过期时展示正文；`withdrawn` / `taken_down` / `expired` 展示对应的中性占位。渲染 MUST 实时判定（不缓存正文），MUST NOT 暴露发送者的精确位置或 openid。收件箱 SHALL 定期自动刷新。

#### Scenario: 发送者撤回后收件人看到占位

- **WHEN** 发送者撤回一条已投递的祝福
- **THEN** 收件人的收件箱里那条不再显示正文，改为"这份祝福已被收回"占位

### Requirement: 回一段祝福

收件人 SHALL 可对一条收到的祝福"回一段祝福"（`scope = reply`）。回复的受众恒为原发送者一人，MUST 同样经过内容校验后投递。系统 MUST NOT 提供任何形式的即时聊天 / 会话线程——回复是一条独立的、走完整状态机的祝福。收件人 MUST NOT 回复自己。

#### Scenario: 回复进入对方收件箱

- **WHEN** 收件人对一条祝福点"回一段祝福"并提交
- **THEN** 通过校验后，原发送者的收件箱出现这条回复祝福，并收到通知

#### Scenario: 没有会话

- **WHEN** 双方来回回了几段祝福
- **THEN** 每一段都是发件箱 / 收件箱里的独立条目，没有聊天窗口或消息线程
