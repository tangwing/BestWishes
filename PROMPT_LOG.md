...
### B-94 demo 走查反馈：改名、祈福必须为他人而写、浏览/回复计数

> 我们继续完善产品。最新demo我已经看过，需要修改的
> 1. 按照我们的规范，如果需要，就更新Spec等。
> 2. 一定要严格遵守我们的质量要求来完善。
> 以下为优化点：
> 1. 把"传递善意"改为"传播善意"，因为它有一种广播的感觉。
> 2. 比较大的一个改动是，我在思考之后，我认为祈福广场上面刊登的祈福应该是必须要是为他人祈福，而不是为自己来寻找祝福的。所以我们要想办法把这个机制能够框到我们产品当中去。这个想法合理吗？符合我们的理念和初衷吗？把我们的理念和初衷也明确加到我们的产品中，看看加在哪里合适。
> 3. 从UI的角度上，我们是，我不太清楚是我们应该现在就面向移动端去设计我们的UI，还是说等到功能全部测试完成之后。这个听一下你的建议。
> 4. 在空间中，为了鼓励大家传播善意，或者让大家对善意有更具体的体感，可以增加一些数字呈现。我们目前有"几次善意"，未来应该增加的是这些善意本身被浏览的次数、被回复的次数等等。你可以进一步设计这一点，让大家更有体感：我传播的善意，其实有其他人关注到。

**结果**：四点都在同一个未归档的 `redesign-kindness-entry`（B-94）change 里追加落地（B-97~B-100，详见 CHANGELOG / BACKLOG 恢复点）。

对第 2 点先做了判断再动手：**合理**——`concept.md` 的首发切入点原文就是"为他人/逝者的祈福"，`Home.tsx` 也早已是"写给具体的人的祝福"，唯独 `PublishWishRequest.tsx` 的实际文案一直在引导"为自己求安慰"，这是没对齐的地方，不是新方向。机制上刻意不做语义判断（"是否真的在说别人"不可靠、属过度设计），改用结构性引导：撰写祈福新增必填的 `beneficiaryLabel`（"TA是谁"），处境描述围绕"TA"重写。理念声明加进了祈福广场入口和发起祈福页，没有另开一个"关于"页（超出本轮范围）。`vision.md`"祝福请求方"persona 的措辞同步改写。

对第 4 点，实现过程中发现并修正了一个自己刚引入的设计缺陷：最初把 `viewCount` 自增直接放进 `detail()` / `getPublicPage()`，但这两个详情页都按 3 秒轮询等待新内容出现——若轮询本身也计数，浏览数字会被无意义刷高，体感数字反而失真。改为客户端首次打开页面时单独调一次 `recordView`/`recordPublicView`，与轮询的 `GET` 请求分离；作者查看自己的不计数；数字只对作者本人可见。

对第 3 点是直接给建议、没有动手实现：三步路线的顺序是刻意定的（先动线、再可分享产物、最后手机形态），现在插队做移动端专项设计会在 B-95 落地后被迫返工。

`pnpm verify`（224 测试）/ `pnpm test:e2e`（17 个）/ `openspec validate --all --strict`（17 项）全绿，`docs/DEMO.md` 已补四点变化的走查。
